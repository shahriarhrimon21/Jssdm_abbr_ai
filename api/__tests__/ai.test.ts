 ...
 * ../../server/ai/handler.ts). What's specific to this file is: does it
 * actually export that same handler (not a stale copy), and does it declare
 * the Edge runtime Vercel needs to run a Web-standard Request/Response
 * handler unmodified.
 */
...
import sharedHandler from "../../server/ai/handler.ts";
